import { ColourVariants } from '../src/configuration/colours/getColours';
import { SortByType, SortOrderType, ViewLayoutType } from '../src/pages/Home';
import getApiUrl from '../src/configuration/getApiUrl.server';
import defaultHeaders from '../src/configuration/defaultHeaders';
import getCookie from '../src/functions/getCookie';
import getFetchCredentials from '../src/configuration/getFetchCredentials';
...
const updateUserConfig = async (config: UserConfigType): Promise<UserMeType> => {
  const { data: userConfig } = await http.post('/api/user/me/', {
    config,
  });
